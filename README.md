...
- Connects directly to a backup endpoint (S3 / S3-compatible, Backblaze
  B2, Azure Blob, Google Cloud Storage, SFTP, WebDAV, local disk, and
  the OAuth providers Google Drive / OneDrive / Dropbox).
...
Grab the installer for your OS and CPU, **verify the checksum**, then install.
Full instructions (all platforms, uninstall, WSL notes) are in
...
`scripts/` - Windows `.msi`, Linux `.deb` / `.rpm`, macOS `.pkg`, each in
x86-64 and ARM64.  Cross-building the ARM64 installers from an x86-64 host
has extra host setup; see `docs/BUILD_ARM64.md`.
