...
then an optional **review** (a context-aware quality pass with Claude Opus).
...
## Step 2 (optional) - Claude Opus context-aware review
...
### Billing: the Anthropic API is NOT Claude Pro / Max

Two separate Anthropic products share a login but **not a budget**:

| Product | Powers | Billing |
|---|---|---|
| Claude Pro / Max | claude.ai chat, Claude Code | flat monthly subscription |
| Anthropic API (console.anthropic.com) | scripts like `opus_review.py` | pay-as-you-go per token |

`opus_review.py` spends API credit, which starts at zero and is separate from
any Pro / Max subscription. For a hard cap: buy a small prepaid amount
(e.g. $5) at console.anthropic.com and **leave auto-recharge off** - calls
fail with a no-credit error rather than overshooting. A `--context-only` pass
across all locales costs well under a dollar; a full pass is ~$10-15.
...
Translations are machine-generated (free Google endpoint) with a Claude Opus
...
- `scripts/i18n/opus_review.py` - Claude Opus context-aware review pass.
