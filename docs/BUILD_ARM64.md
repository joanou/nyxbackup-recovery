...
| macOS ARM64   | `.pkg` | `scripts/macos/build_recover_pkg_arm64.sh` |
...
`aarch64-unknown-linux-gnu` cargo/cc cross variables.  libssh2 is built from
source via libssh2-sys's `vendored` feature (no system ARM64 libssh2 needed);
`libssl-dev:arm64` covers openssl-sys.
