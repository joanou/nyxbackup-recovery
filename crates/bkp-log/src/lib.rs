...
impl SizeRollingAppender {
    ...
    fn rotate(&mut self) -> io::Result<()> {
        ...
        // Cycle existing archives: .N.gz is deleted, .N-1.gz becomes .N.gz
        ...
        // Delete the oldest if it already exists (because the loop above only
        // moves .keep-1 into .keep, but if .keep exists from a previous cycle
        // it needs to be removed before rename).
        let oldest = archive_path(&self.log_path, self.keep);
        if oldest.exists() {
