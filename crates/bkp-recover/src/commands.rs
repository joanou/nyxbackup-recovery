...
fn endpoint_to_toml(p: &EndpointParams) -> String {
    shared_endpoint_to_toml(&params_to_endpoint_config(p))
}
...
pub async fn rec_test_connection(args: ConnectArgs) -> Result<(), String> {
    ...
    match tokio::time::timeout(TEST_TIMEOUT, backend.list("manifests/")).await {
        ...
        Ok(Err(e)) => {
            ...
            return Err(format!("test connection: {e}"));
        }
        Err(_) => {
            tracing::warn!("rec_test_connection: timed out");
            return Err(format!(
                "test connection timed out after {}s - check the endpoint URL, \
                 credentials, region, and that the bucket is reachable",
...
pub async fn rec_unlock(
    ...
    let bytes = hex_decode(hex).map_err(|e| format!("invalid master key: {e}"))?;
    if bytes.len() != 32 {
        return Err(format!(
...

    {
        ...
        if s.endpoint.is_none() {
            return Err("not connected; complete the Connect step first".into());
        }
...
pub async fn rec_list_snapshot_files(
    ...
    let set_id =
        BackupSetId::from_uuid(Uuid::parse_str(&args.set_id).map_err(|e| format!("set_id: {e}"))?);
    let snapshot_id = bkp_types::snapshot::SnapshotId::from_uuid(
        Uuid::parse_str(&args.snapshot_id).map_err(|e| format!("snapshot_id: {e}"))?,
    );
...
pub async fn rec_start_restore(
    ...
    if !dest.exists() {
        std::fs::create_dir_all(&dest)
            .map_err(|e| format!("create dest dir {}: {e}", dest.display()))?;
    }
        ...
        let pack_map = match bkp_restore::build_pack_map_from_storage(&*throttled).await {
            ...
            Err(e) => {
                ...
                if let Some(ref mut v) = s.progress {
                    v.status = "error".into();
                    v.error_detail = format!("build pack map: {e}");
                }
                return;
            }
        };
        ...
        match result {
            ...
            Err(e) => {
                ...
                if let Some(ref mut v) = s.progress {
                    ...
                    v.error_detail = if cancelled { String::new() } else { msg };
                    v.paused = false;
                }
            }
        }
