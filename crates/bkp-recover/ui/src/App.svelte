...
<script lang="ts">
  ...
  // Tauri window is built with decorations=true + titleBarStyle="Overlay"
  // via tauri.macos.conf.json, the OS draws traffic lights upper-left,
  // and Settings / About move to the native NSMenu wired in
  // src/bin/gui.rs.  The in-window ⚙ + ? + min / max / close buttons
  // are suppressed entirely on macOS - they exist only for Win + Linux.
...
<!-- titleBarStyle "Visible" (Mac default) means the OS-drawn title bar
     ...
     padding needed. -->
<div class="flex flex-col h-screen bg-nyx-bg text-nyx-text">
