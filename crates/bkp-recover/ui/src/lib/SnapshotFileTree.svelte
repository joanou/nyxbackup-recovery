...
-->
<script lang="ts">
  import { t, tf } from './i18n.svelte.ts'
...
>
  ...
  <div class="shrink-0 px-3 py-1.5 border-t border-nyx-border bg-nyx-surface text-xs text-nyx-muted">
    ↑↓ navigate · ▶/▼ or click 📁 to expand · checkbox to select · Space toggle · right-click for options
  </div>
  ...
  {#if contextMenu}
    ...
    >
      ...
      {#if onRestoreFile}
        ...
        >Restore this {contextMenu.entry.is_dir ? 'folder' : 'file'}</button>
      {/if}
      <button
        role="menuitem"
        onclick={ctxCopyPath}
