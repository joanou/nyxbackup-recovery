...
<script lang="ts">
  ...
  const STORAGE_LABELS: Record<EndpointType, string> = {
    local:         'Local folder / mounted share (NFS, USB, ...)',
    s3:            'Amazon S3',
    ...
    webdav:        'WebDAV (Nextcloud, Synology, etc.)',
  }
  ...
  function validateForm(): string | null {
    if (!connectForm.storageUrl.trim()) return 'Storage URL / path is required'
    if (KEY_ID_BACKENDS.includes(connectForm.storageType)
        && !connectForm.storageKeyId.trim()) return 'Key / username is required'
    ...
    if (connectForm.storageType === 's3_compat' && !connectForm.storageRegion.trim()) {
      return 'Endpoint URL is required for S3-compatible backends (e.g. https://s3.wasabisys.com)'
    }
  ...
  async function handleTestConnection() {
    ...
    try {
      ...
      connTested = 'ok'
      connMessage = 'Connection verified.'
    } catch (e) {
      connTested = 'fail'
      connMessage = String(e)
    } finally {
      testing = false
    }
  }
...
<div class="max-w-3xl mx-auto p-6 flex flex-col gap-5">
  <header class="text-center">
    ...
    <p class="text-xs text-nyx-muted mt-1">
      The Recovery Tool reads from any Nyx Backup endpoint without
      requiring the main app or a license.  Credentials and the master
      key live in memory only - nothing is persisted to disk.
    </p>
  </header>
  ...
  <section class="rounded-lg border border-nyx-border bg-nyx-surface p-5 flex flex-col gap-3">
    ...
    <div class="flex flex-col gap-1.5">
      <label for="rec-storage-url" class="text-xs text-nyx-muted">
        {connectForm.storageType === 'google_drive' ? 'Google Drive folder URL or folder ID'
          : (connectForm.storageType === 'onedrive' || connectForm.storageType === 'dropbox') ? 'Folder path'
          : 'URL / path'}
      </label>
      <input
        id="rec-storage-url"
        ...
        {:else}
          ...
          <div class="flex flex-col gap-1">
            <label for="rec-secret" class="text-xs text-nyx-muted">
              {connectForm.storageType === 's3' || connectForm.storageType === 's3_compat' ? 'Secret access key'
                : connectForm.storageType === 'azure_blob' ? 'Account key'
                : 'Application key'}
            </label>
            <input
              id="rec-secret"
    ...
    {#if connectForm.storageType === 'google_drive' || connectForm.storageType === 'onedrive' || connectForm.storageType === 'dropbox'}
      <div class="flex flex-col gap-2">
        ...
        {#if connectForm.storageType === 'onedrive'}
          <div class="flex flex-col gap-1">
            <label for="rec-onedrive-tenant" class="text-xs text-nyx-muted">Account type</label>
            <select
              ...
              class="bg-nyx-surface2 border border-nyx-border rounded-lg px-3 py-2 text-sm
                     text-nyx-text focus:outline-none focus:border-nyx-accent transition-colors"
            >
              <option value="common">Any Microsoft account (personal + work/school)</option>
              <option value="consumers">Personal Microsoft account only</option>
              <option value="organizations">Work or school account</option>
              <option value="custom">Specific Entra tenant (GUID)</option>
            </select>
          ...
          >{connectingOnedrive ? 'Connecting...' : 'Connect with Microsoft'}</button>
        {/if}

        <!-- Manual (no-local-browser) sign-in relay - works for all 3 providers. -->
        <details class="text-[10px] text-nyx-muted">
          <summary class="cursor-pointer">No browser on this machine? (manual sign-in)</summary>
          <div class="mt-1.5 flex flex-col gap-2">
            {#if connectForm.storageType === 'google_drive'}
              <p class="text-[10px] text-nyx-muted">
                Enter your Drive folder URL/ID in the field above first.
              </p>
            {/if}
            <button
              type="button"
            ...
            {#if manualAuthUrl}
              <p class="text-[10px] text-nyx-muted">
                1. Open this link in any browser (phone, another PC) and approve:
              </p>
              <textarea
                readonly
                rows="2"
              ...
              ></textarea>
              <p class="text-[10px] text-nyx-muted">
                2. Your browser then lands on a "can't reach localhost" page - copy
                that page's full address (it contains <span class="font-mono">?code=...</span>)
                and paste it here:
              </p>
              <input
              ...
              >{manualBusy ? 'Finishing...' : 'Finish sign-in'}</button>
            {/if}
          </div>
        </details>
        ...
        <details class="text-[10px] text-nyx-muted">
          ...
          <div class="mt-1.5 flex flex-col gap-1">
            ...
            />
            <p class="text-[10px] text-nyx-muted">
              On the source machine, inspect the keyring entry named
              <span class="font-mono">nyxbackup.ep-NNNN.secret</span>
    ...
    <div class="flex flex-col gap-1">
      <label for="rec-label" class="text-xs text-nyx-muted">Label (optional)</label>
      <input
        id="rec-label"
