...
<script lang="ts">
  ...
  async function pickDestination() {
    try {
      const sel = await openDialog({ directory: true, title: 'Select restore destination' })
  ...
  async function startRestore() {
    ...
    if (!destPath) {
      restoreErr = destType === 'custom'
        ...
        : 'Could not determine a default restore folder - choose Custom and pick one'
      return
    }
      ...
      pollTimer = setInterval(async () => {
        try {
          ...
          if (p && (p.status === 'complete' || p.status === 'error' || p.status === 'cancelled')) {
            ...
            else restoreErr = p.error_detail || 'restore failed'
          }
        } catch (e) {
          if (pollTimer) clearInterval(pollTimer)
          pollTimer = null
  ...
  async function pauseRestore()  { try { await api.pauseRestore()  } catch (e) { restoreErr = String(e) } }
  async function resumeRestore() { try { await api.resumeRestore() } catch (e) { restoreErr = String(e) } }
  async function cancelRestore() { try { await api.cancelRestore() } catch (e) { restoreErr = String(e) } }
  ...
  <div class="flex flex-col bg-nyx-surface border border-nyx-border rounded-lg overflow-hidden">
    ...
    <div class="flex-1 overflow-auto">
      ...
      {:else if snapshots.length === 0}
        <p class="px-3 py-2 text-xs text-nyx-muted">No snapshots reachable with this key.</p>
      {:else}
        {#each snapshots as snap (snap.snapshot_id)}
          ...
          >
            ...
            <div class="text-[10px] text-nyx-muted mt-0.5">
              {Number(snap.files_total).toLocaleString()} files · {humanBytes(Number(snap.bytes_total))}
            </div>
          </button>
        {/each}
      {/if}
  ...
  <div class="flex flex-col bg-nyx-surface border border-nyx-border rounded-lg overflow-hidden
    ...
    <div class="px-3 py-2 border-b border-nyx-border bg-nyx-surface">
      ...
      {#if fileSelection.size > 0}
        <span class="ml-2 text-nyx-accent text-xs">({fileSelection.size} selected)</span>
      {/if}
      {#if selectionLocked}
        <span class="ml-2 text-[10px] text-nyx-muted">(locked while restoring)</span>
      {/if}
    </div>
    <div class="flex-1 overflow-auto">
      {#if loadingFiles}
  ...
  <div class="flex flex-col bg-nyx-surface border border-nyx-border rounded-lg p-4 gap-3">
      ...
      <label class="flex flex-col gap-0.5 cursor-pointer">
        <span class="flex items-center gap-2 text-sm">
          ...
          <span class="text-nyx-text">
            Default folder{#if defaultDestBase}: <span class="font-mono text-xs">{defaultDestBase}</span>{/if}
          </span>
        </span>
      </label>
    ...
    {#if restoreDone}
      <div class="flex flex-col gap-1.5">
        ...
        <button onclick={() => api.openFolder(destPath).catch((e) => { restoreErr = String(e) })}
