...
///
/// Resolution rule (matches `bkp-daemon::keystore::key_file_path` and the
/// CLAUDE.md "Linux/macOS paths" table):
///
...
async fn apply_file_metadata(dest: PathBuf, mtime_ns: u64, owner: Arc<RestoreOwner>) {
    if mtime_ns == 0 && owner.owner_sid.is_empty() && owner.unix_uid == 0 && owner.unix_gid == 0 {
        return;
    }
...
impl RestoreEngine {
    ...
    /// directory.  For example, if the backup set covers `/home/joanou`, a file
    /// at `/home/joanou/Documents/file.txt` restores to `{dest}/Documents/file.txt`
    /// instead of `{dest}/home/joanou/Documents/file.txt`.
    ...
    pub async fn restore_all(
        ...
        let mut raw_files: Vec<(PathBuf, Vec<ChunkRef>, u64, Option<String>)> = Vec::new();
        ...
        for (rel_path, chunks, mtime_ns, symlink_target) in raw_files {
        ...
        // - Opt 3: pre-create all destination directories -----------
        {
            ...
            // Linux/macOS: `~/NyxRestore-2026-...` and `~/.../joanou/`
            ...
            // Outside the dest root we skip - `/home/joanou` /
...
async fn write_file_with_pack_cache(
    ...
    // the attribute couldn't be applied.
    apply_file_metadata(dest.clone(), mtime_ns, owner).await;
    Ok(())
}
...
/// Source `/home/joanou` + file `/home/joanou/Documents/foo.txt`
/// produced `Documents/foo.txt`; now it produces `joanou/Documents/foo.txt`.
...
/// (e.g. `/home/joanou` and `/home/joanou/Documents`) strip the more
...
fn strip_source_prefix<'a>(file_path: &'a str, source_prefixes: &[String]) -> &'a str {
    ...
    for prefix in source_prefixes {
            ...
            && p.len() >= best_match_len
        {
            ...
            // survives.  E.g. p=`/home/joanou`, parent_len=6,
            // normalized=`/home/joanou/Documents/foo.txt` ->
            // `joanou/Documents/foo.txt`.
...
fn collect_files(
    node: &TreeNode,
    prefix: &Path,
    out: &mut Vec<(PathBuf, Vec<ChunkRef>, u64, Option<String>)>,
) {
    ...
    match node.node_type {
        ...
        NodeType::File | NodeType::Symlink => {
            if let Some(entry) = &node.file_entry {
                let sym = if matches!(node.node_type, NodeType::Symlink) {
                    entry.symlink_target.clone()
                } else {
                    None
                };
                out.push((path, entry.chunks.clone(), entry.mtime_ns, sym));
            }
        }
    }
}
