...
mod tests {
    ...
    // decode_machine_record) is covered by the format-conformance fixture
    // described in docs/DESIGN.md "Format parity", not by encode->decode
    // round-trips - this crate is decode-only and has no encode path to
    // round-trip against.
